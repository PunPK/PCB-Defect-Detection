...
export default function ImageUpload() {
    ...
    useEffect(() => {
        ...
        const storedRight = sessionStorage.getItem('rightImage')

        if (storedLeft) setoriginlImage(JSON.parse(storedLeft))
        if (storedRight) setRightImage(JSON.parse(storedRight))
    }, [])
    ...
    useEffect(() => {
        ...
        if (rightImage) {
            sessionStorage.setItem('rightImage', JSON.stringify(rightImage))
        } else {
            sessionStorage.removeItem('rightImage')
        }
    }, [originlImage, rightImage])
    ...
    const handleFiles = (fileList, side) => {
        ...
        if (side === 'left') {
            ...
            if (rightImage) URL.revokeObjectURL(rightImage.url) // Clean up previous image
    ...
    return (
        <div className="flex flex-col md:flex-row justify-center items-center gap-8 p-4">
            ...
            <div className="w-full max-w-sm bg-white rounded-lg shadow-lg overflow-hidden border border-blue-500">
                <div className="bg-blue-500 text-white font-medium py-3 px-4">Upload Originl Image</div>
            ...
            {/* Right Upload Panel */}
            <div className="w-full max-w-sm bg-white rounded-lg shadow-lg overflow-hidden border border-blue-500">
                <div className="bg-blue-500 text-white font-medium py-3 px-4">Upload Image (Right)</div>
                    ...
                    {!rightImage ? (
                        ...
                        <div className="mt-4">
                            <div className="flex flex-col items-center">
                                <div
                                    className="relative group cursor-pointer"
                                    onClick={() => openPreview(rightImage)}
                                >
                                    <img
                                        src={rightImage.url}
                                        alt={rightImage.name}
                                ...
                                <div className="flex items-center justify-between w-full mt-2 bg-gray-50 p-2 rounded-md">
                                    <span className="text-sm text-gray-700 truncate flex-1">{rightImage.name}</span>
                                    <button
